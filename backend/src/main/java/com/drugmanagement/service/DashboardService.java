...
import com.drugmanagement.dto.DashboardResponse;
...
public class DashboardService {
    ...
    public DashboardResponse getDashboardData(String warehouseId, String category, String status) {
        ...
        if (category != null && !category.trim().isEmpty() && !category.equalsIgnoreCase("all")) {
            try {
                com.drugmanagement.enums.Category categoryEnum = com.drugmanagement.enums.Category.valueOf(category.toUpperCase());
        ...
        List<ProductBatch> batches = productBatchRepository.findByWarehouseId(warehouseId);
    ...
    private List<DashboardResponse.StockItem> getStockData(List<Product> products, String warehouseId) {
    ...
    private List<DashboardResponse.LowStockAlert> getLowStockAlerts(List<Product> products, String warehouseId) {
        return products.stream()
            .filter(p -> p.getQuantity() < p.getThresholdQuantity())
            .limit(10) // Top 10 low stock alerts
            .map(product -> new DashboardResponse.LowStockAlert(
                product.getProductName(),
                product.getSkuId(),
                product.getQuantity(),
